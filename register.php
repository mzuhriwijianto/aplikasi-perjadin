...
if(isset($_POST['submit'])){
    ...
    if($error == '') {
        ...
        $mail->SMTPSecure = 'tls';
        $mail->SMTPAuth = true;
        $mail->Username = "mzuhriw@gmail.com";
        $mail->Password = "logingmail;";
        $mail->setFrom('no-reply-aplikasi-perjadin@bojonegoro.go.id', 'Konfirmasi Registrasi Aplikasi Perjadin');
        //Set the subject line
        $mail->Subject = 'PHPMailer GMail SMTP test';
        //Read an HTML message body from an external file, convert referenced images to embedded,
        //convert HTML into a basic plain-text alternative body
        $mail->msgHTML(file_get_contents('http://localhost:8080/aplikasi-perjadin/email-confirm.php?id_pegawai='.$id_pegawai.'&pass='.$passbaru.''), dirname(_FILE_));
        //Replace the plain text body with one created manually
 
        $mail->Subject    = 'Konfirmasi Register';
